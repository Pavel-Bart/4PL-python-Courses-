...
import models
import schemas
from repository import student_repository as repo
...
def update(id: int, request: schemas.StudentPostSchema, db: Session = Depends(get_db)):
...
@router.post('')
def create(request: schemas.StudentPostSchema, db: Session = Depends(get_db)):
